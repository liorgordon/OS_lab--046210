...
NORET_TYPE void do_exit(long code)
{
	...
	/* free TODO list from task_struct when exiting process */
	struct list_head *pos, *q;
	todo_stack* tmp;
	if (tsk->TODO_list != NULL) {
		list_for_each_safe(pos, q, &tsk->TODO_list->list) {
			tmp = list_entry(pos, todo_stack, list);
			list_del(pos);
